 ...
 */
export const deleteTask = async (taskId: string) => supabase.from('tasks').delete().eq('id', taskId)
...
export const storeReport = async (report: string) => {
	...
	const { error } = await supabase.from('documents').insert({ content: report, embedding })
